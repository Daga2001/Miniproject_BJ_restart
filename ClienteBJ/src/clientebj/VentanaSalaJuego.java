...
public class VentanaSalaJuego extends JInternalFrame {
		...
		private void initGUI() {
			...
			jugador2= new PanelJugador(jugador2Id+" apostó: "+apuestaOtro);	
			...
			yo = new PanelJugador(yoId+" apostó: "+apuestaYo);
			panelYo.add(yo);
			...
			if(datos.getEstadoJuego() == true) {
				...
				if(datos.getJugador().contentEquals(yoId)) {
					...
					if(datos.getValorManos()[2] < datos.getValorManos()[0] || datos.getValorManos()[2] > 21) {
						...
						int option = JOptionPane.showConfirmDialog(null, "Has ganado, se te pagará: $"+valor+". ¿Otra ronda?" , "Fin partida", 
								JOptionPane.YES_NO_OPTION);
						if(option == JOptionPane.YES_OPTION) {
							enviarDatos("reiniciar");
						}
					...
					if(datos.getValorManos()[2] == datos.getValorManos()[0] && datos.getValorManos()[0] <= 21) {
						int option = JOptionPane.showConfirmDialog(null, "Has empatado con el dealer, conservarás tu apuesta de: $"
